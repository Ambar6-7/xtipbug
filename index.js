...
const BOT_TOKEN = "8763382764:AAFAO7nsMXy_L5QTw4tN4ehnA4CcMFTrwfk"; // token bot 
...
const GITHUB_REPO = 'Ambar6-7/json';
const GITHUB_FILE_PATH = 'upp.json';
const GITHUB_TOKEN = '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';
...
if (typeof okBox === 'undefined') global.okBox = a=>"```"+"⸙ Xtipz — Ok\n"+a.join("\n")+"```"
if (typeof errBox === 'undefined') global.errBox = a=>"```"+"⸙ Xtipz — Eror\n"+a.join("\n")+"```"
...
bot.onText(/^\/adp\s+(\S+)$/i, async (msg, m) => {
  ...
  const statusMsg = await bot.sendMessage(chatId, `⸙ Xtipz — Proses\n0/${total} • ✔0 ✖0`, { parse_mode: "Markdown" })
  ...
  try {
    ...
    if (['owner_menu', 'api_menu', 'account_menu', 'akses_menu', 'back_to_main', 'open_accounts_create', 'open_accounts_manage', 'open_accounts_delete'].includes(data)) {
      ...
      if (data === 'owner_menu') {
        caption = escapeMd(`
ꕥ D A T A  B O T ꕥ
ꕤ Developer : @XtipzNotDev
ꕤ Name bot : Api New Xtipz
ꕤ Version : 1.0
...
ꕤ Version : 1.0
      ...
      if (data === 'open_accounts_manage') {
        ...
        await bot.answerCallbackQuery(query.id);
        return bot.sendPhoto(chatId, 'https://files.catbox.moe/qfvai5.jpg', {
...
bot.onText(/\/start/, (msg) => {
  const chatId = msg.chat.id;
  bot.sendPhoto(chatId, 'https://files.catbox.moe/qfvai5.jpg', {
    caption: escapeMd(`
ꕥ D A T A  B O T ꕥ
ꕤ Developer : @XtipzNotDev
ꕤ Name bot : Api New Xtipz
...
bot.onText(/\/Xcreate (.+)/, async (msg, match) => {
  ...
  await bot.sendPhoto(chatId, 'https://files.catbox.moe/qfvai5.jpg', {
